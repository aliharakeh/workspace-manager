...
| Ship it | `bun run compile` → `release/app-runner.exe` | `bun run build:stable` / `build:canary` via Electrobun |
...
bun run compile      # standalone binary → release/app-runner(.exe)
```
...
bun run build:stable # Electrobun production build
```
